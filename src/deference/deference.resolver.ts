...
export class DeferenceResolver {
    ...
    createDeference(@Args('createDeferenceInput') createDeferenceInput: CreateDeferenceInput) {
        return this.deferenceService.createDeference(createDeferenceInput);
    }

    @Mutation((returns) => DeferenceResponse)
    updateDeference(@Args('updateDeferenceInput') updateDeferenceInput: UpdateDeferenceInput) {
